...
export function BudgetProfilePanel({
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="left-0 top-0 translate-x-0 translate-y-0 w-full max-w-full h-screen rounded-none sm:left-[50%] sm:top-[50%] sm:translate-x-[-50%] sm:translate-y-[-50%] sm:max-w-5xl sm:h-auto sm:max-h-[92vh] sm:rounded-lg overflow-x-hidden overflow-y-auto p-0 gap-0">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Card>
                <CardContent className="p-3 sm:p-4">
                  ...
                  <p className="text-base sm:text-xl font-bold mt-1 text-emerald-700">{fmtUSD(derivedMonthlyIncome)}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-3 sm:p-4">
                  ...
                  <p className="text-base sm:text-xl font-bold mt-1 text-red-700">{fmtUSD(monthlyExpenses)}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    ...
                    <span className="truncate">Monthly Net</span>
                  </div>
                  <p className={`text-base sm:text-xl font-bold mt-1 ${monthlyNet >= 0 ? "text-emerald-700" : "text-red-700"}`}>
              ...
              <Card>
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    ...
                    <span className="truncate">Total Debt</span>
                  </div>
                  <p className="text-base sm:text-xl font-bold mt-1 text-purple-700">{fmtUSD(totalDebtBalance)}</p>
                </CardContent>
              </Card>
            </div>
                ...
                <CardContent className="p-2 sm:p-4 space-y-4">
                  ...
                  <table className="w-full text-sm">
                    <thead className="text-xs text-muted-foreground bg-muted/30">
                      <tr>
                        <th className="text-left px-2 sm:px-3 py-2 font-medium">Debt</th>
                        <th className="hidden sm:table-cell text-left px-3 py-2 font-medium">Type</th>
                        <th className="text-right px-2 sm:px-3 py-2 font-medium">Balance</th>
                        <th className="hidden sm:table-cell text-right px-3 py-2 font-medium">APR</th>
                        <th className="text-right px-2 sm:px-3 py-2 font-medium">Min/mo</th>
                        <th className="hidden sm:table-cell text-left px-3 py-2 font-medium">Paid Off</th>
                      </tr>
                    </thead>
                    <tbody>
                      {debts
                        ...
                        .map(d => (
                          <tr key={d.id} className="border-t">
                            <td className="px-2 sm:px-3 py-2 font-medium text-xs sm:text-sm leading-tight">
                              {d.name}
                              <span className="sm:hidden block text-[10px] text-muted-foreground font-normal">
                                {d.interestRate != null ? `${d.interestRate}% APR · ` : ""}{DEBT_TYPE_LABEL[d.type] ?? d.type}
                              </span>
                            </td>
                            ...
                            <td className="px-2 sm:px-3 py-2 text-right font-semibold text-red-700 text-xs sm:text-sm">
                              {fmtUSD(d.balance)}
                            </td>
                            <td className="hidden sm:table-cell px-3 py-2 text-right">
                              {d.interestRate != null ? `${d.interestRate}%` : "—"}
                            </td>
                            <td className="px-2 sm:px-3 py-2 text-right text-xs sm:text-sm">{fmtUSD(debtMonthlyPayment(d))}</td>
                      ...
                      <tr className="border-t bg-muted/20">
                        <td className="px-2 sm:px-3 py-2 font-semibold text-xs sm:text-sm">Total</td>
                        <td className="hidden sm:table-cell px-3 py-2"></td>
                        <td className="px-2 sm:px-3 py-2 text-right font-bold text-red-700 text-xs sm:text-sm">{fmtUSD(totalDebtBalance)}</td>
                        <td className="hidden sm:table-cell px-3 py-2"></td>
                        <td className="px-2 sm:px-3 py-2 text-right font-bold text-xs sm:text-sm">{fmtUSD(totalMonthlyDebtPayments)}</td>
                        <td className="hidden sm:table-cell px-3 py-2"></td>
                      </tr>
                    </tbody>
                  </table>
