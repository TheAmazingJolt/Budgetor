...
import { Banknote, ChevronRight, TrendingDown } from "lucide-react";
...
export function PaydayCheckInDialog({
  ...
  const handleConfirm = async () => {
    ...
    setError("");
    try {
      await apiFetch(`/api/budgets/${budgetId}/payday-checkins`, {
        method: "POST",
        ...
        {step === 1 ? (
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Week of {weekStart} — here's where your paycheck goes.
              </DialogDescription>
            </DialogHeader>
              ...
              {runningRows.length === 0 ? (
                ...
                <div className="space-y-1.5 max-h-[40vh] overflow-y-auto pr-0.5">
                  {runningRows.map((row, i) => (
                    <div
                      key={i}
                      className="rounded-lg border border-muted/60 bg-background px-3 py-2.5 flex items-center justify-between gap-2"
                    >
