...
router.post("/budget/generate", async (req, res): Promise<void> => {
  ...
  const { startDate, endDate, openingBalance, paycheckAmount, numberOfWeeks, bills, payPeriod, incomeSources, priorSavings } = parsed.data as typeof parsed.data & { priorSavings?: Record<string, Record<string, number>> };
